...
const DashboardOverview: React.FC<DashboardOverviewProps> = ({ onNavigate }) => {
  ...
  const kpiData = [
    { label: 'Total Claims', value: stats?.total_claims?.toString() || '...', change: '+24', color: 'text-white' },
    { label: 'Verified %', value: stats ? `${stats.verified_percentage}%` : '...', change: '+1.2%', color: 'text-primary' },
    { label: 'High-Risk Flags', value: stats?.high_risk_flags?.toString() || '...', change: '-1', color: 'text-red-500' },
    { label: 'Pending Audits', value: stats?.pending_audits?.toString() || '...', change: 'Stable', color: 'text-yellow-500' }
  ];
